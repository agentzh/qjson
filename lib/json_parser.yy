...
number: int { $$ = QVariant ($1.toInt()); }
        | int fract {  $$ = QVariant ($1.toString().append($2.toString()).toDouble()); }
        | int exp {  $$ = QVariant ($1.toString().append($2.toString()).toDouble()); }
        | int fract exp {
...
string: QUOTMARKOPEN string_arg QUOTMARKCLOSE {
          $$ = QVariant(QString::fromUtf8($2.toByteArray()));
      };

string_arg: /*empty */ { $$ = QVariant (""); }
            | string_arg WORD {
                QByteArray bytes = $1.toByteArray();
                bytes += $2.toByteArray();
                $$ = QVariant(bytes);
              };
